// CLAUDENOTE: real-mouse-stroke profiler, DYNTOPO variant. Same input
// synthesis as mouse-stroke-profile.mjs, but triangulates the mesh, enables
// dyntopo on the active brush, and reports remesh stats alongside the
// per-RAF frame/dab timings.
//
// Usage: node mouse-stroke-dyntopo.mjs [--port=9777]
...
// --- 1. setup: show window, triangulate, enable dyntopo, frame mesh ---
const setup = await evalJs(`
  ...
  t.frameMeshInCamera();
  const tm = _appstate.ctx.scene.toolmode;
  const brush = tm.getBrush();
  // The mousedown handler rebuilds the brush copy's dynTopoSC from the
  // TOOLMODE defaults (loadDefaults + overrideMask), so enable it there.
  tm.dynTopoSC.flag |= 1; // DynTopoFlagsSC.ENABLED
  brush.dynTopoSC.flag |= 1;
  tm.resetDynTopoStats();
  const addon = globalThis.__nativeManager && globalThis.__nativeManager.addon;
  if (addon && addon.redirectStdout) {
    addon.redirectStdout('C:/Users/joeed/AppData/Local/Temp/claude/C--dev-webgl-app-framework/6054124d-7ec1-45d9-a1c1-1054bb74496e/scratchpad/scprof_dyntopo.txt');
  }
  ...
  return {rafOk, hidden: document.hidden, triangulated,
          verts: mesh.mesh.v.count,
          tool: brush.tool, radiusPx: brush.radius,
          dt: {edgeMode: brush.dynTopoSC.edgeMode, edgeSize: brush.dynTopoSC.edgeSize,
               maxSplits: brush.dynTopoSC.maxSplits, spacing: brush.dynTopoSC.dynTopoSpacing},
          pos: [...v3d.pos], size: [...v3d.size]};
`)
...
const result = await evalJs(`
  ...
  return {
    ...
    dynTopoStats: tm.dynTopoStats,
    vertsAfter: t.mesh.mesh.v.count,
  };
`)
