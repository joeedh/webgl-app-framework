...
export class ElementListIter<type extends Element> {
  ...
  next(): IteratorResult<type> {
    const ret = this.ret
    const elist = this.elist
    const list = elist.list
...
mesh.ElementList {
  ...
  swap(a: type, b: type): this {
    ...
    if (i1 < 0) throw new Error('element not in array ' + a)
    if (i2 < 0) throw new Error('element not in array ' + b)
  ...
  _remove(e: type, no_error = false): void {
    ...
    if (i >= 0) {
      ...
      if (this.storeFreedElems) {
        ...
        const index = this.delayed_free_queue.length
  ...
  remove(elem: type, no_error = false): this {
    ...
    if (this.active === elem) this.active = undefined
    if (this.highlight === elem) this.highlight = undefined
  ...
  selectNone(): this {
    for (const elem of this.selected) {
  ...
  loadSTRUCT(reader: StructReader<this>) {
    ...
    const act = this.active as unknown as number
    const high = this.highlight as unknown as number
    ...
    //old file?
    const oldthis = this as unknown as any
  ...
  removeCustomDataLayer(layer_i: number): void {
    ...
    const layer = this.customData.flatlist[layer_i]
  ...
  _runDelayedFreeQueue(): void {
    ...
    for (const elem of this.delayed_free_queue) {
      if (elem === undefined) {
        continue
      }
  ...
  _checkFreeElems(): void {
    ...
    for (const elem of this.free_elems.queue) {
      if (!elem) {
        continue
      }
  ...
  addCustomDataLayer<CDType extends CustomDataElem<unknown> = CustomDataElem<unknown>>(
    ...
    for (const item of this) {
      if (haveOnNewLayer) {
        for (const cd of item.customData) {
          if (cd.onNewLayer) {
  ...
  fixCustomData(): void {
    ...
    for (const e of this) {
      ...
      for (const k in cd.layers) {
        ...
        for (const cdl of e.customData) {
          if (cdl.typeName === layerset.typeName) {
            count++
          }
        }

        const typecls = CustomDataElem.getTypeClass(layerset.typeName)
  ...
  stripTempLayers(saveState = false): any {
    ...
    const layers = cdata.flatlist
    const map = new Array<number | undefined>(layers.length)
    let j = 0
    ...
    for (const elem of this) {
      const customData = elem.customData
