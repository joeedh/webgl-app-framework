...
import {BVHVertFlags, getDynVerts} from './bvh.js'
...
export function cleanupQuadsOld(mesh, faces, lctx) {
  ...
  for (let f of faces) {
    ...
    if (len === 4) {
      ...
      //strategy one: collapse loops between tris
      let stop = false
      ...
      //strategy two: expand edge loops between tris
...
export function remeshMesh(
  mesh,
  remesher = Remeshers.UNIFORM_TRI,
  lctx = undefined,
  goalType,
  goalValue,
  maxSteps = 5,
