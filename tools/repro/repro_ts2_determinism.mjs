...
async function replayAndGrab() {
  ...
  await page.evaluate(async () => { await window._appstate.ctx.replay(() => true) })
...
console.log(`replay1 vs replay2: sizeA=${ma.size} sizeB=${mb.size} diff=${diff} worst=${worst.toFixed(5)} @v${worstIdx} onlyA=${onlyA} onlyB=${onlyB}`)
