...
const res = await page.evaluate(() => {
  ...
  let qEqU = 0, qOther = 0, createdInStep = 0, worst = 0, worstIdx = -1
  const diffs = []
  for (const [idx, p] of P) {
    ...
    if (d <= 1e-6) continue
    if (d > worst) { worst = d; worstIdx = idx }
    const u = U.get(idx)
    if (!u) createdInStep++ // vert absent after undo => created during step 3
    if (eq(q, u)) qEqU++; else qOther++
    if (diffs.length < 6) diffs.push({idx, p: p.map(x=>+x.toFixed(4)), u: u ? u.map(x=>+x.toFixed(4)) : null, q: q.map(x=>+x.toFixed(4)), d: +d.toFixed(4)})
  }
...
console.log('P!=Q verts: Q==U (redo no-op)=' + res.qEqU + '  Q==other=' + res.qOther + '  createdInStep(absent@U)=' + res.createdInStep)
console.log('worst=' + res.worst.toFixed(5) + ' @v' + res.worstIdx)
console.log('samples:')
for (const d of res.diffs) console.log('  v' + d.idx + ' d=' + d.d + ' P=' + JSON.stringify(d.p) + ' U=' + JSON.stringify(d.u) + ' Q=' + JSON.stringify(d.q))
