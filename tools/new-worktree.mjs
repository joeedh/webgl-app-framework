...
if (!name) die('usage: node tools/new-worktree.mjs <name> [--base <ref>] [--branch <branch>]')
branch = branch || name
...
Write-Host "sccache env: BASEDIRS=$env:SCCACHE_BASEDIRS DIR=$env:SCCACHE_DIR"
...
echo "sccache env: BASEDIRS=$SCCACHE_BASEDIRS DIR=$SCCACHE_DIR"
...
console.log('\nteardown when done:')
console.log(`  git -C ${MAIN} worktree remove ${DEST}`)
