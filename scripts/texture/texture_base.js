      ...
      for (let k in list1) {
        ...
        if (!k in slist) {
          if (typeof v === "object" && v instanceof Array) {
            ...
            v = v.copy();
          } else if (typeof v === "object" && v.clone) {
            v = v.clone();
          }
        } else {
...
float fsample(vec3 co, vec3 colorOut) {
  ...
  loadSTRUCT(reader) {
    ...
    this.uniforms = JSON.parse(this.uniforms);

    for (let i=0; i<2; i++) {
      let list = i ? this.uniforms : this.params;
...
`;
nstructjs.register(TextureShader);
