...
- **Random:** A random distribution of points may be added by entering the required number of points and clicking the _"Generate random vertices"_ button. The points can be sampled from a uniform or standard normal distribution.
- **Import from file:** Points may also be imported from a file using the button under _"Load vertices from file"_. The x and y coordinates of each point should be space- or comma-separated, and each point should exist on its own line.
- **Manual entry:** Directly add or modify points using the text-box under _"Vertex list"_, and click the _"Reload input data"_ button.

2. Specify the set of edges to constrain, if any.

- **Random:** Click the _"Generate random edge constraints"_ button to randomly generate the specified number of edge constraints.
- **Import from file:** Edge constraints can also be imported from a file using the button under _"Load constrained edges from file"_. Each edge should be specified by the indices to its two end-points in the vertex list. The vertex indices use 0-based indexing, and should be space- or comma-separated.
- **Manual entry:** Directly add or modify edge constraints using the text-box under _"Constrained edge list"_, and click the _"Reload input data"_ button.

3. Click the _"Triangulate!"_ button to generate the constrained Delaunay triangulation.
