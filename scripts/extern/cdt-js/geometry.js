...
function isEdgeIntersectingAtEndpoint(edgeA, edgeB) {
  const rsq_tol = 1e-13
  if (edgeA[0].sqDistanceTo(edgeB[0]) < rsq_tol) return true

  if (edgeA[0].sqDistanceTo(edgeB[1]) < rsq_tol) return true

  if (edgeA[1].sqDistanceTo(edgeB[0]) < rsq_tol) return true

  if (edgeA[1].sqDistanceTo(edgeB[1]) < rsq_tol) return true
