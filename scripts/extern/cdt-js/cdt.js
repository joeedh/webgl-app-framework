...
window.onload = function () {
  ...
  canvas.onmousedown = function (e) {
    onCanvasMouseDown(canvas, e)
  }
  canvas.onmousemove = function (e) {
    onCanvasMouseMove(canvas, e)
  }
  canvas.onmouseup = function (e) {
    onCanvasMouseUp(canvas, e)
  }
  canvas.onwheel = function (e) {
    onCanvasMouseWheel(canvas, e)
  }
...
function loadInputData() {
  ...
  printToLog(
    'Loaded ' + globalMeshData.vert.length + ' vertices and ' + globalMeshData.con_edge.length + ' constrained edges.'
  )
...
function genRandVertices() {
  ...
  if (document.getElementById('radiouniform').checked) {
    for (let i = 0; i < nVert; i++) content += Math.random().toFixed(10) + ', ' + Math.random().toFixed(10) + '\n'
  } else {
    for (let i = 0; i < nVert; i++) content += randn(0, 1).toFixed(10) + ', ' + randn(0, 1).toFixed(10) + '\n'
  }
...
function genRandEdges() {
  ...
  while (edge_list.length < txt.value && iter < maxIter) {
    ...
    if (new_edge[0] === new_edge[1]) continue

    if (!isEdgeValid(new_edge, edge_list, globalMeshData.vert)) continue

    edge_list.push(new_edge)
  }

  var content = ''
  for (let i = 0; i < edge_list.length; i++) content += edge_list[i][0] + ', ' + edge_list[i][1] + '\n'
...
function transformCoord(coord) {
  const x =
    (((coord.x - min_coord.x) / screenL) * canvas_L + canvas_translation.x) * zoom_scale +
    0.5 * (canvas_width - canvas_L * zoom_scale)
  const y =
    canvas_height -
    (((coord.y - min_coord.y) / screenL) * canvas_L - canvas_translation.y) * zoom_scale -
...
function invTransformCoord(coord) {
  const x =
    (((coord.x - 0.5 * (canvas_width - canvas_L * zoom_scale)) / zoom_scale - canvas_translation.x) * screenL) /
      canvas_L +
    min_coord.x
  const y =
    (((canvas_height - coord.y - 0.5 * (canvas_height - canvas_L * zoom_scale)) / zoom_scale + canvas_translation.y) *
      screenL) /
      canvas_L +
    min_coord.y
  return new Point(x, y)
...
function renderCanvas(forceRender) {
  ...
  if (!forceRender && performance.now() - last_render_time < render_dt) return
  ...
  if (render_vertices_flag) renderVertices(ctx, globalMeshData)
  ...
  if (point_loc_search_path.length > 0) drawPath(point_loc_search_path)
...
function renderVertices(ctx, meshData) {
  ...
  for (let i = 0; i < meshData.vert.length; i++) {
    ...
    if (isPointVisible(canvas_coord)) ctx.fillRect(canvas_coord.x - 2, canvas_coord.y - 2, 4, 4)
  }
}
...
function renderSelectedVertex(ctx, ind_vert) {
  ...
  document.getElementById('div_info').innerHTML =
    '<b>Vertex:</b> <br>&nbsp &nbsp Index: ' + ind_vert + '<br>&nbsp &nbsp Coordinates: ' + coord.toStr()
}
...
function renderSelectedTriangle(ctx, ind_tri) {
  ...
  for (let adj_tri = 0; adj_tri < 3; adj_tri++) {
    ...
    if (ind_adj_tri == -1) continue

    if (adj_str == '') adj_str = ind_adj_tri
    else adj_str += ', ' + ind_adj_tri
  ...
  document.getElementById('div_info').innerHTML =
    ...
    '<br>&nbsp &nbsp Index: ' +
    ind_tri +
    '<br>&nbsp &nbsp Vertex indices: ' +
    triangles[ind_tri][0] +
    ', ' +
    triangles[ind_tri][1] +
    ', ' +
    triangles[ind_tri][2] +
    '<br>&nbsp &nbsp Vertex coords: ' +
    verts[triangles[ind_tri][0]].toStr() +
    ', ' +
    verts[triangles[ind_tri][1]].toStr() +
    ', ' +
    verts[triangles[ind_tri][2]].toStr() +
    '<br>&nbsp &nbsp Adjacent triangle indices: ' +
    adj_str
}
...
function isEdgeVisible(p0, p1) {
  ...
  return p_min.x < canvas_width && p_max.x >= 0 && p_min.y < canvas_height && p_max.y >= 0
}
...
function onCanvasMouseMove(canvas, e) {
  ...
  if (isMouseDown && performance.now() - last_mousedown_time > 100) {
    //left button clicked
    canvas_translation.x += (last_canvas_coord.x - mouse_down_coord.x) / zoom_scale
    canvas_translation.y += (last_canvas_coord.y - mouse_down_coord.y) / zoom_scale
    renderCanvas(false)
...
function onCanvasMouseWheel(canvas, e) {
  ...
  if (e.deltaY < 0) zoom_scale *= 1.05
  else if (e.deltaY > 0) zoom_scale *= 0.952380952
...
function displayTriangulationInfo(canvas, e) {
  ...
  if (selected_vertex_index == -1 && triangles.length > 0) {
    ...
    if (ind_tri >= 0) selected_triangle_index = ind_tri
  }
...
function locateVertex() {
  ...
  if (ind == '' || ind < 0 || ind >= globalMeshData.vert.length) return
...
function locateTriangle() {
  ...
  if (ind == '' || ind < 0 || ind >= globalMeshData.tri.length) return
...
function triangulate() {
  ...
  if (document.getElementById('checkboxConstrain').checked && globalMeshData.con_edge.length > 0) {
    t0 = performance.now()
  ...
  } else printToLog('Computed Delaunay triangulation in ' + t_delaunay.toFixed(2) + ' ms.')
...
function findEnclosingTriangleOld(target_vertex, meshData, ind_tri_cur) {
  ...
  while (!found_tri && nhops < max_hops) {
    ...
    const bary_coord = barycentericCoordTriangle(
      target_vertex,
      vertices[tri_cur[0]],
      vertices[tri_cur[1]],
      vertices[tri_cur[2]]
    ...
    } else if (bary_coord.s >= 0.0 && bary_coord.t >= 0.0 && bary_coord.u >= 0.0) {
      found_tri = true
    }
...
function findEnclosingTriangleSlow(target_vertex, meshData, ind_tri_cur) {
  ...
  for (let ind_tri = 0; ind_tri < triangles.length; ind_tri++) {
    ...
    if (getPointOrientation([vertices[tri_cur[0]], vertices[tri_cur[1]]], target_vertex) < 0) continue

    if (getPointOrientation([vertices[tri_cur[1]], vertices[tri_cur[2]]], target_vertex) < 0) continue

    if (getPointOrientation([vertices[tri_cur[2]], vertices[tri_cur[0]]], target_vertex) < 0) continue
...
function removeBoundaryTriangles(meshData) {
  ...
  //Update adjacencies
  for (let i = 0; i < adjacency.length; i++) for (let j = 0; j < 3; j++) adjacency[i][j] = indmap[adjacency[i][j]]
...
function isDelaunay(v_tri, p) {
  ...
  else if (cos_a < 0 && cos_b < 0) return false

  const sin_ab = (vec02.x * vec12.y - vec12.x * vec02.y) * cos_b + (vec1p.x * vec0p.y - vec0p.x * vec1p.y) * cos_a

  if (sin_ab < 0) return false
  else return true
}
...
function buildVertexConnectivity(meshData) {
  ...
  for (let itri = 0; itri < triangles.length; itri++) {
    for (let node = 0; node < 3; node++) {
      if (vConnectivity[triangles[itri][node]] == undefined) vConnectivity[triangles[itri][node]] = [itri]
...
function checkCDT() {
  ...
  for (let iedge = 0; iedge < con_edges.length; iedge++) {
    ...
    if (!edge_in_triangulation) throw 'Edge ' + iedge + ' is not in the triangulation!'
  }
  ...
  for (let itri = 0; itri < triangles.length; itri++) {
    ...
    for (let indv = 0; indv < verts.length; indv++) {
      if (indv == cur_tri[0] || indv == cur_tri[1] || indv == cur_tri[2]) continue

      if (ccenter.sqDistanceTo(verts[indv]) > rsq) continue //skip points outside circumcircle
      ...
      for (let edge_t = 0; edge_t < 3; edge_t++) {
        ...
        if (getPointOrientation(tri_edge_verts, verts[indv]) >= 0) continue //skip edge if vertex if on left (triangle edges go anticlockwise)
        ...
        for (let edge_c = 0; edge_c < con_edges.length; edge_c++) {
          ...
          const edge0_invisible_to_vert =
            isEdgeIntersecting(edge0_to_vert, con_edge_verts) &&
            !isEdgeIntersectingAtEndpoint(edge0_to_vert, con_edge_verts)

          const edge1_invisible_to_vert =
            isEdgeIntersecting(edge1_to_vert, con_edge_verts) &&
            !isEdgeIntersectingAtEndpoint(edge1_to_vert, con_edge_verts)

          if (
            con_edges[edge_c][0] != tri_edge_node0 &&
            con_edges[edge_c][1] != tri_edge_node0 &&
            con_edges[edge_c][0] != tri_edge_node1 &&
            con_edges[edge_c][1] != tri_edge_node1
          ) {
          ...
          if (
            (con_edges[edge_c][0] == tri_edge_node0 || con_edges[edge_c][1] == tri_edge_node0) &&
        ...
        if (is_blocked_by_con_edge) {
          //one of the nodes of this edge can't see vertex i
