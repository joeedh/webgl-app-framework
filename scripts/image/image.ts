...
`);
  ...
  _promises: Array<(image: any) => void>
  _image: any;
  ...
  _regen() {
    ...
    if (this.type === ImageTypes.URL) {
      ...
      image.onload = () => {
        ...
        (image as undefined as any).image = g.getImageData(0, 0, canvas.width, canvas.height);
        this._firePromises();
      }
...
ImageUser {
  ...
  dataLink(ownerBlock: DataBlock<any, any>, getblock, getblock_addUser) {
