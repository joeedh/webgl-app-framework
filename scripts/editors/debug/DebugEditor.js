...
export const DrawShaders = {
  IDS: {
    //key should map to one of DisplayModes keys
...
export class DebugEditor extends Editor {
  ...
  drawStart(gl) {
    ...
    let x = this.owning_sarea.pos[0] * dpi,
      y = (this.owning_sarea.pos[1] + this.owning_sarea.size[1]) * dpi
  ...
  rect(gl, tex, uniforms = {}) {
    ...
    if (this.displayMode === DisplayModes.RAW) mesh.program = Shaders.DisplayShader
    else if (this.displayMode === DisplayModes.DEPTH) mesh.program = Shaders.DepthShader
...
DebugEditor.STRUCT =
  nstructjs.inherit(DebugEditor, Editor) +
  `
  displayMode      : int;
