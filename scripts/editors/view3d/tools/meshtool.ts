...
export class MeshToolBase extends ToolMode {
  ...
  findHighlight(e: any, x: number, y: number, selectMask: number = this.selectMask): {elem: any; mesh: any} | undefined {
    ...
    if (found) {
      ...
      let redraw: boolean = mesh.getElemList(elem.type).highlight !== elem;
  ...
  on_mousemove(e: any, x: number, y: number, was_touch: boolean): boolean | any {
    ...
    if (mdown) {
      let dist: number = this.last_mpos.vectorDistance(this.start_mpos);
  ...
  drawIDs(view3d: any, gl: WebGL2RenderingContext, uniforms: any, selmask: any = undefined): void {
    ...
    let camdist: number = view3d.activeCamera.pos.vectorDistance(view3d.activeCamera.target);
    ...
    for (let mesh of resolveMeshes(this.ctx, this.getMeshPaths())) {
      ...
      let program: any = Shaders.MeshIDShader;
  ...
  drawSphere(gl: WebGL2RenderingContext, view3d: any, p: Vector3, scale: number = 0.01, color: number[] = [1, 0.4, 0.2, 1.0]): void {
  ...
  on_drawend(view3d: any, gl: WebGL2RenderingContext): void {
    ...
    for (let mesh of resolveMeshes(this.ctx, this.getMeshPaths())) {
      ...
      let program: any = Shaders.MeshEditShader;
