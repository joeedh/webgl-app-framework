...
import type {Mesh, Vertex, Edge, Face, Element} from '../../../../addons/builtin/mesh/src/mesh.js'
...
export class FindnearestMesh extends FindnearestClass {
  ...
  static castScreenCircle(ctx: ViewContext, selmask: number, mpos: IVectorOrHigher<2>, radius: number, view3d: View3D) {
    ...
    if (selmask & SelMask.GEOM) {
      objects = new Set(ctx.selectedObjects).filter((ob) => ob.data?.lib_type === 'mesh')
    } else {
      objects = new Set(ctx.scene.objects.editable).filter((ob) => ob.data?.lib_type === 'mesh')
