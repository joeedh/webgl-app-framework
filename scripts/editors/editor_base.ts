...
export class NewDataBlockOp<InputSet = {}, OutputSet = {}> extends ToolOp<
  InputSet &
  {
    name: StringProperty,
...
export class CopyDataBlockOp<InputSet = {}, OutputSet = {}> extends ToolOp<
  InputSet &
  {
    block: DataRefProperty,
    dataPathToSet: StringProperty
...
export class AssignDataBlock<InputSet = {}, OutputSet = {}> extends ToolOp
  <
    InputSet &
    {
      block: DataRefProperty,
...
export class UnlinkDataBlockOp<InputSet = {}, OutputSet = {}> extends ToolOp<
  InputSet &
  {
    block: DataRefProperty,
...
export class Editor extends Area<ViewContext> {
...
Editor {
  ...
  static defineAPI(api: DataAPI): DataStruct {
    ...
    st.string("type", "type", "Type", "Editor type").customGetSet(function (this: { dataref: Editor }) {
...
App {
  ...
  updateCanvasSize() {
    ...
    let size = this.size, canvas = document.getElementById("webgl") as HTMLCanvasElement;
...
window.setInterval(() => {
  if (window._appstate && window._appstate.ctx) {
    window._appstate.ctx.messagebus.validateSubscribers();
  }
}, 5000);
