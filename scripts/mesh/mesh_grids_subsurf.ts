...
export class PatchBuilder {
  ...
  build() {
    ...
    let mesh = this.mesh = oldmesh.copy(false);
    ...
    for (let v of mesh.verts) {
      let v2 = oldmesh.eidMap.get<Vertex>(v.eid);
    ...
    for (let l of mesh.loops) {
      ...
      let uvs = [[0, 0], [0, 1], [1, 1], [1, 0]].map(f => new Vector2(f));
    ...
    for (let [eid, f] of lmap) {
      let l = oldmesh.eidMap.get<Loop>(eid);
