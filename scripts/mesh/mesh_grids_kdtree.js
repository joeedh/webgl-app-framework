...
const QFLAG          = KdTreeFields.QFLAG,
      ...
      QCHILD3        = KdTreeFields.QCHILD3,
      QCHILD4        = KdTreeFields.QCHILD4,
...
CompressedKdNode.fields = makeCompressedNodeStruct().fields;
CompressedKdNode.STRUCT = makeCompressedNodeStruct().nstruct;
nstructjs.register(CompressedKdNode);
...
const {SELECT, LEAF, DEAD, TEMP} = KdTreeFlags;
...
export class KdTreeGrid extends GridBase {
  ...
  _ensureNodePoint(ni, pidx, loopEid = undefined, mesh, isNewOut) {
    let nodes = this.nodes;
  ...
  evaluate(u, v, startNi = 0, depthLimit = undefined) {
    ...
    a.load(p1).interp(p2, v2);
    b.load(p4).interp(p3, v2);
    a.interp(b, u2);
  ...
  buildTangentMatrix1(ni, u1, v1, matOut) {
    let m = matOut.$matrix;
    ...
    if (0 && this.subsurf) {
      ...
      */
      ...
      if (0 && this.subsurf) {
        ...
        a.load(r1).interp(r2, v1);
        b.load(r4).interp(r3, v1);
        a.interp(b, u1);
...
//*
  ...
  subdivideAll(mesh, loop, cd_grid) {
    ...
    this.subdivideAll_intern(mesh, loop, cd_grid, true);
  ...
  subdivideAll_intern(mesh, loop, cd_grid) {
    let ns = this.nodes;
    let nodes = [];
    ...
    */
    ...
    let rec = (ni) => {
      ...
      if (ni === 0) {
        ...
        let split = ns[ns[ni+QPARENT]+QSPLIT];
      }
  ...
  checkNodeNormals() {
    ...
    for (let ni of this.getLeafNodes()) {
      ...
      t1.load(p4).sub(p1);//.normalize();
      t2.load(p3).sub(p2);//.normalize();
      ...
      //*
      t1.load(p2).sub(p1);//.normalize();
      t2.load(p3).sub(p4);//.normalize();
      t1.interp(t2, 0.5);//.normalize();
      //*/
  ...
  uvColorTest(mesh, loop, cd_grid) {
    ...
    for (let ni of this.getLeafNodes()) {
      for (let i = 0; i < 4; i++) {
        ...
        let color = p.customData[cd_col].color;
