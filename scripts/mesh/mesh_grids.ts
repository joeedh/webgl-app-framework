...
export class NeighborMap {
  ...
  maps: {
    [p: number]:
      ...
      | ((number | number)[] | (number | number)[] | (undefined | number)[])[]
  }
  ...
  constructor(dimen: number) {
    ...
    function lmask(a: keyof typeof masks, b: keyof typeof masks) {
...
mesh.GridVert {
  ...
  addTan(ns: number[], ni: number[], pidx: number) {
    //this.tot++;
  }
  ...
  load(b: IBVHVertex | Vector3 | this, coOnly = true) {
    if (!b) {
      return
    }
    ...
    if (erased instanceof GridVertBase || erased instanceof Vertex || erased instanceof Handle) {
      ...
      console.warn('Unreachable codepath detected')
      bco = new Vector3()
    }
...
mesh.GridBase {
  ...
  static calcCDLayout(mesh: Mesh) {
    ...
    for (let layer of mesh.loops.customData.flatlist) {
      ...
      if (ok) {
        cdlayers.push([i, cls as unknown as ICustomDataElemConstructor])
      }
  ...
  initCDLayoutFromLoop(loop: Loop): void {
    ...
    for (let i = 0; i < this.cdmap.length; i++) {
      this.cdmap[i] = undefined as unknown as number
    }

    for (let i = 0; i < this.cdmap_reverse.length; i++) {
      this.cdmap_reverse[i] = undefined as unknown as number
    }
  ...
  loadSTRUCT(reader: StructReader<this>): void {
    ...
    for (let i = 0; i < this.customDatas.length; i++) {
      ...
      if (!cls) {
        //add a dummy class to maintain data structure integrity
