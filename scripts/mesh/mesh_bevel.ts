...
export function splitEdgeLoops_pre(
  mesh: Mesh,
  edgesIter: Iterable<Edge> = mesh.edges,
  vertWidthMap = new Map(),
  width = 0.25,
  lctx?: LogContext
) {
...
export function splitEdgeLoops(
  mesh: Mesh,
  edgeIter: Iterable<Edge> = mesh.edges,
  vertWidthMap = new Map(),
  width = 0.25,
  lctx?: LogContext
) {
    ...
    for (let v of e.verts) {
      for (let e2 of v.edges) {
        ...
        for (let l2 of e2.loops) {
          ...
          let v1: Vertex | undefined, v2: Vertex | undefined
  ...
  return {dirMap: dirmap, cornerMap: origmap2, origEdgeMap, origVertMap}
}
